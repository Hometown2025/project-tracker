...
class TaskManagerTester:
    ...
    def test_auto_populate_subtasks_functionality(self):
        ...
        # Test authentication with both admin and super admin
        super_admin_login = {
            "username": "superadmin",
            "password": "superadmin123",
            "store_id": "GLOBAL"
        ...
        super_admin_response = self.test_request("POST", "/auth/login", super_admin_login, 200, "Super Admin Login for Subtasks Testing")
        super_admin_token = super_admin_response.get('session_token') if super_admin_response else None
        ...
        demo_response = self.test_request("POST", "/auth/login", demo_login, 200, "Demo User Login for Access Control Test")
        demo_token = demo_response.get('session_token') if demo_response else None
        
        if demo_token and created_rooms:
            test_room = created_rooms[0]
            self.test_request("POST", f"/tasks/{test_room['id']}/generate-subtasks", {}, 403, 
                            "Demo User Generate Subtasks (Should Fail)", auth_token=demo_token)
        ...
        if super_admin_token:
            ...
            super_admin_room = self.test_request("POST", "/tasks", super_admin_room_task, 200, "Create Room for Super Admin Access Test", auth_token=super_admin_token)
            
            if super_admin_room:
                super_admin_subtasks = self.test_request("POST", f"/tasks/{super_admin_room['id']}/generate-subtasks", {}, 200, 
                                                        "Super Admin Generate Subtasks", auth_token=super_admin_token)
    ...
    def test_new_room_type_subtask_generation(self):
        ...
        if not self.admin_token:
            self.log("❌ No admin token available for room type subtask testing", "ERROR")
            return
        
        if not self.test_data['projects']:
            self.log("❌ No projects available for room type testing", "ERROR")
            return
