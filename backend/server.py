...
async def get_projects(current_user: User = Depends(get_current_user)):
    ...
    if current_user.role == UserRole.SUPER_ADMIN:
        ...
        # Users only see projects they're assigned to from their store
...
async def get_tasks(project_id: Optional[str] = None, status: Optional[TaskStatus] = None, current_user: User = Depends(get_current_user)):
    ...
    for task in tasks:
        ...
        # Add subtask counts for main tasks (level 0)
        if task.get("subtask_level", 0) == 0:
    ...
    if task_completed:
        if updated_task.get("project_id"):
            ...
            await send_notification_to_admins(
                NotificationType.TASK_COMPLETED,
                f"{task_type} Completed",
                f"{task_type} '{updated_task['title']}' has been completed by {current_user.username}"
            )
    else:
        if updated_task.get("project_id"):
            await send_notification_to_project_members(
                NotificationType.TASK_UPDATED,
                f"{task_type} Updated",
                f"{task_type} '{updated_task['title']}' in {project_name} has been updated by {current_user.username}",
                updated_task["project_id"],
                task_id
            )
        else:
